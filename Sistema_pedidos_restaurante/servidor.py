...
def handle_client(client_socket):
    ...
    try:
        restaurante = Restaurante(carta) # Crear instancia de restaurante para este cliente
        ...
        while True:
            try:
                ...
                if opcion == "1":
                    ...
                    pedido_enviado = enviar_pedido_a_cocina_y_salir(client_socket, restaurante)# Devuelve True si se envió un pedido a la cola
...
def mostrar_pedido(client_socket, restaurante):
    try:
        ...
        if pregunta == 's':
            ...
            if producto:
                restaurante.tomar_pedido(nombre, producto, cantidad, observaciones)
                client_socket.sendall(restaurante.mostrar_pedidos().encode())# Enviamos la lista de pedidos actualizada
            else:
                client_socket.sendall("Error: Producto no encontrado en la carta.".encode())
...
def modificar_pedido(client_socket, restaurante):
    try:
        if restaurante.mostrar_pedidos() == "No hay pedidos.": # Verificamos si hay pedidos
