...
def handle_client(client_socket):
    ...
    if '::ffff:' in client_socket.getpeername()[0]:  # Verificar si la dirección IP es IPv4 mapeada en IPv6
        print(f"Conexión establecida con el cliente {client_socket.getpeername()}-ipv4")
    else:
        print(f"Conexión establecida con el cliente {client_socket.getpeername()}-ipv6")
    while True:
    ...
    #-------------Modificar pedido---------------------------------------- #  
        ...
        #-------------Enviar pedido a cocina y salir(resetear pedidos)--------------------------------------- # 
            ...
            if pregunta.lower() == 's':
                ...
                enviar_pedido_a_cocina(restaurante.mostrar_pedidos())
...
def enviar_pedido_a_cocina(pedidos):
    ...
    cocina_socket.sendall(pedidos.encode())
    cocina_socket.close()

def server():
    HOST = '::'  # Escucha en todas las interfaces IPv4 e IPv6
    PORT = 50007
    s = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)  # Usa AF_INET6 para IPv6
    s.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_V6ONLY, 0)  # Permite conexiones IPv4 mapeadas en IPv6
    s.bind((HOST, PORT))
    s.listen(5)
...
def server():
    HOST_IPV4 = '0.0.0.0'  # Escucha en todas las interfaces IPv4
    HOST_IPV6 = '::'        # Escucha en todas las interfaces IPv6
    PORT = 50007

    # Socket para IPv4
    s_ipv4 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s_ipv4.bind((HOST_IPV4, PORT))
    s_ipv4.listen(5)

    # Socket para IPv6
    s_ipv6 = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
    s_ipv6.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_V6ONLY, 1)  # Solo IPv6, sin IPv4 mapeado
    s_ipv6.bind((HOST_IPV6, PORT))
    s_ipv6.listen(5)

    while True:
        # Aceptar conexiones de ambos sockets
        conn_ipv4, addr_ipv4 = s_ipv4.accept()
        conn_ipv6, addr_ipv6 = s_ipv6.accept()

        # Crear un hilo para manejar cada conexión
        threading.Thread(target=handle_client, args=(conn_ipv4,)).start()
        threading.Thread(target=handle_client, args=(conn_ipv6,)).start()
